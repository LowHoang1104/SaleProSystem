...
/**
 * Get total revenue from registered customers only (excluding guest transactions)
 */
...
/**
 * Get REAL Average Order Value from Invoice table
 * This is the actual AOV, not average customer spending
 */
...
public Map<String, Integer> getCustomersByRank() {
    ...
    String sql = "SELECT ISNULL(Rank, 'Thường') as RankName, COUNT(*) as Count " +
                "FROM Customers " +
                "WHERE CustomerID != 1 " +  // Exclude "Khách lẻ"
                "GROUP BY Rank";
    try {
        stm = connection.prepareStatement(sql);
        rs = stm.executeQuery();
...
/**
 * Get customers for reports with filtering (excluding guest customer by default)
 */
